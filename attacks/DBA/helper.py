...
class Helper:
    def __init__(self, current_time, params, name):
        ...
        # PCA-Deflect additions
        if self.params.get('enable_client_history_tracking', False):
            self.client_param_history = {}  # Store client model parameters (clean baseline)
            self.client_clean_history = {}  # Store clean model parameters before flagging
            self.flagged_malicious_clients = set()  # Track detected malicious clients
            self.outlier_history = {}  # Track outlier detection history
    ...
    def accumulate_weight(self, weight_accumulator, epochs_submit_update_dict, state_keys,num_samples_dict):
        ...
        if self.params['aggregation_methods'] == config.AGGR_FOOLSGOLD:
            ...
            updates = dict()
            for i in range(0, len(state_keys)):
                # print("Available keys in epochs_submit_update_dict:", list(epochs_submit_update_dict.keys()))
                # print("Current state_keys[i]:", state_keys[i])
    ...
    def average_shrink_models(self, weight_accumulator, target_model, epoch_interval, agents_num):
        ...
        With PCA-Deflect trust factor support.
        """
        print("lengthof agents:" ,agents_num)
        
        # PCA-Deflect: Apply trust factor if enabled
        if (self.params.get('aggregation_methods') == config.AGGR_PCA_DEFLECT and 
            self.params.get('enable_client_history_tracking', False) and 
            hasattr(self, 'flagged_malicious_clients') and 
            len(self.flagged_malicious_clients) > 0):
            
            print(f"[PCA-DEFLECT] Applying trust factor to {len(self.flagged_malicious_clients)} flagged clients")
            trust_factor = self.params.get('trust_factor', config.PCA_DEFLECT_TRUST_FACTOR)
            
            # Note: In actual implementation, we'd need to track which updates
            # come from which clients and apply trust factor selectively
    ...
    def update_client_history(self, client_id, model_state_dict):
        """Update client parameter history for PCA-Deflect."""
        if self.params.get('enable_client_history_tracking', False):
            # Always update current history
            self.client_param_history[client_id] = copy.deepcopy(model_state_dict)
            
            # If client is not yet flagged, save as clean baseline
            if client_id not in self.flagged_malicious_clients:
                self.client_clean_history[client_id] = copy.deepcopy(model_state_dict)
                print(f"[PCA-DEFLECT] Saved clean baseline for client {client_id}")
    
    def get_client_history(self, client_id):
        """Get client parameter history."""
        return self.client_param_history.get(client_id, None)
    
    def get_client_clean_history(self, client_id):
        """Get client clean parameter history (before being flagged)."""
        return self.client_clean_history.get(client_id, None)
    
    def update_outlier_history(self, client_id, epoch):
        """Track outlier detection history."""
        if client_id not in self.outlier_history:
            self.outlier_history[client_id] = []
        self.outlier_history[client_id].append(epoch)
        
        # Check if immediate flagging is enabled
        immediate_flagging = self.params.get('immediate_flagging', config.PCA_DEFLECT_IMMEDIATE_FLAGGING)
        
        if immediate_flagging:
            # Flag immediately when detected as outlier
            if client_id not in self.flagged_malicious_clients:
                self.flagged_malicious_clients.add(client_id)
                print(f"[PCA-DEFLECT] Client {client_id} flagged immediately as outlier detected")
        else:
            # Use original persistence-based flagging
            min_rounds = self.params.get('min_rounds_before_flagging', 
                                         config.PCA_DEFLECT_MIN_ROUNDS_BEFORE_DETECTION)
            persistence = self.params.get('outlier_persistence_rounds', 
                                         config.PCA_DEFLECT_OUTLIER_PERSISTENCE)
            
            if len(self.outlier_history[client_id]) >= persistence:
                recent_detections = self.outlier_history[client_id][-persistence:]
                if max(recent_detections) - min(recent_detections) <= persistence:
                    self.flagged_malicious_clients.add(client_id)
                    print(f"[PCA-DEFLECT] Client {client_id} flagged as malicious after {persistence} detections")
    
    def flag_client_immediately(self, client_id):
        """Immediately flag a client as malicious."""
        if client_id not in self.flagged_malicious_clients:
            self.flagged_malicious_clients.add(client_id)
            print(f"[PCA-DEFLECT] Client {client_id} immediately flagged as malicious")
            return True
        return False
    
    def is_client_flagged(self, client_id):
        """Check if client is flagged as malicious."""
        return client_id in self.flagged_malicious_clients
    
    def get_flagged_clients(self):
        """Get all flagged malicious clients."""
        return list(self.flagged_malicious_clients)
    
    def update_detection_metrics(self, metrics):
        """Update detection metrics from PCA-Deflect."""
        if hasattr(self, 'detection_metrics'):
            for key in ['true_positives', 'false_positives', 'total_triggers']:
                self.detection_metrics[key].update(metrics.get(key, {}))
    
    def get_detection_metrics(self):
        """Get current detection metrics."""
        return self.detection_metrics if hasattr(self, 'detection_metrics') else None
    
    def reset_detection_metrics(self):
        """Reset detection metrics for new epoch."""
        if hasattr(self, 'detection_metrics'):
            for key in self.detection_metrics:
                self.detection_metrics[key].clear()
