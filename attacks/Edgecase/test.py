...
def Mytest_poison(helper, epoch,
    ...
    if True:
        ...
        for batch_id, batch in enumerate(data_iterator):
            data, targets, poison_num = helper.get_poison_batch_test(data_iterator, batch, evaluation=True)
            ...
            pred = output.data.max(1)[1]  # get the index of the max log-probability
            # correct += pred.eq(targets.data.view_as(pred)).cpu().sum().item()
            if helper.params['type'] == config.TYPE_MNIST or helper.params['type'] == config.TYPE_EMNIST:
                correct += (pred == helper.params['poison_label_swap']).sum().item()
            else:
                correct += (pred == 9).sum().item()
